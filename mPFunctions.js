...
function mainMenu(error) {
  ...
  if (error == true) {
    console.log(
      "Error: Invalid input. Please select either [1,2,3,4,5,6,7,8 or 9]\n"
    );
  }
    ...
    if (musiName.length >= 3 && musiName.length <= 30) {
      if (musiExp >= 0) {
        if (musiRate >= 50) {
          ...
          musiRate = parseInt(
            prompt("Enter the Musicians hourly rate (50 or greater): ").trim()
          );
          musi.hourlyRate = musiRate;
        }
      } else {
        musiExp = prompt(
          "Enter the Musicians Experience (0 or greater): "
        ).trim();
  ...
  while (true) {
    if (troupeName.length >= 3 && troupeName.length <= 30) {
      if (troupeMinDuration >= 0.5 && troupeMinDuration <= 3) {
        if (
          troupeMaxDuration >= 0.5 &&
          troupeMaxDuration <= 3 &&
          troupeMaxDuration >= troupeMinDuration
        ) {
          ...
          troupeMaxDuration = prompt(
            "Enter the Troupes Maximum Duration (between 0.5 and 3) :"
          ).trim();
          troupe.maxDuration = troupeMaxDuration;
        }
      } else {
        troupeMinDuration = prompt(
          "Enter the Troupes Minimum Duration (between 0.5 and 3) :"
        ).trim();
...
function addMusicianToTroupe() {
  ...
  while (true) {
    if (mus != 0) {
      ...
      musiName = prompt(
        `Type musician name to be added to ${troupe_name} :`
      ).trim();
...
function troupeTimeCost(trou, time) {
  ...
  while (true) {
    if (time <= trou.maxDuration && time >= trou.minDuration) {
      ...
      console.log(
        `\nFor a ${time} hour set at $${trou.totalCost()} an hour, ${
          trou.troupeName
        } will cost $${timeCost}.`
      );
      break;
...
function createImportTroupe() {
  ...
  for (i = 0; list.length > i; i++) {
    ...
    if (list[i].length >= 3 && list[i].length <= 30) {
      ...
      console.log(
        `"${list[i]}" was not registered as "${list[i]}" needs to be 3 to 30 characters.`
      );
    }
  }
}
